  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (globalAlpha > 0) {
        ...
        // Inner glow
        const innerGrad = context.createRadialGradient(
          mouseX,
          mouseY,
          0,
          mouseX,
...
export default function Auth() {
  ...
  try {
    ...
    if (mode === 'login') {
      ...
      // Guardar usuario en localStorage también al registrarse
      if (data.user) {
        localStorage.setItem('gg_token', data.token)
        localStorage.setItem('gg_user', JSON.stringify(data.user))
      }
        ...
        <div className="relative z-10 max-w-[500px]">
          <div className="flex items-center gap-3 mb-12">
            <svg width="32" height="32" viewBox="0 0 28 28" fill="none">
              <path
                d="M14 4 C14 4, 10 8, 10 13 C10 16.3, 12 18.5, 14 19 C16 18.5, 18 16.3, 18 13 C18 8, 14 4, 14 4Z"
                fill="#4ade80"
              />
              <path
                d="M14 19 L14 25"
              ...
              />
              <path
                d="M10 22 C10 22, 8 21, 7 19"
                stroke="#4ade80"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
              <path
                d="M18 22 C18 22, 20 21, 21 19"
                stroke="#4ade80"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
            </svg>
            <span className="font-extrabold text-xs tracking-widest leading-tight whitespace-pre">
              {"GROOVE\nGARDEN"}
            </span>
          </div>
          ...
          <h1 className="font-extrabold text-[clamp(32px,4vw,52px)] leading-[1.1] mb-5">
            Music deserves
            <br />a place to <em className="text-[#4ade80] not-italic">grow</em>
          </h1>
          <p className="text-base text-[#a3b8a5] leading-7 max-w-[440px] mb-12">
            Join a streaming platform where small artists can grow for real.
            Each stream has a real weight.
          </p>

          <div className="flex flex-col gap-6">
            {[
              {
                title: "70% directly to the artist",
                desc: "No intermediaries, just you and your music.",
              },
              {
                title: "Your streams, your impact",
                desc: "Smart distribution based on what you listen to.",
              },
              {
                title: "Community before algorithms",
                desc: "Human-based discoveries",
              },
            ].map((f) => (
        ...
        <div className="w-full max-w-[400px]">
          ...
          <div className="mb-6">
            <h2 className="font-extrabold text-2xl mb-1">
              Welcome{" "}
              <em className="text-[#4ade80] not-italic">
                {mode === "login" ? "back" : "aboard"}
              </em>
            </h2>
            <p className="text-xs text-[#a3b8a5]">
              {mode === "login"
          ...
          {mode === "signup" && (
            <div className="mb-4">
              <label className="block text-xs text-[#a3b8a5] mb-1.5">
                Name:
              </label>
              <input
                type="text"
